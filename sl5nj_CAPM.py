...
spy_daily_return = data["spy_adj_close"].pct_change()
spy_daily_return = spy_daily_return.iloc[1:]
spy_daily_return
appl_daily_return = data["aapl_adj_close"].pct_change()
appl_daily_return = appl_daily_return.iloc[1:]
appl_daily_return

#### 1. (1 PT) Print the first 5 rows of returns

spy_daily_return.head(5)
appl_daily_return.head(5)

spy_daily_rtn = np.array(spy_daily_return)
appl_daily_rtn = np.array(appl_daily_return)
...
spy_rtn = spy_daily_rtn-R_f
appl_rtn = appl_daily_rtn-R_f
...
plt.scatter(spy_rtn,appl_rtn)
plt.xlabel('SPY return')
plt.ylabel('APPL return')
plt.show()
...
spy_rtn_trans = np.transpose(spy_rtn)
spy_rtn_mul = np.matmul(spy_rtn_trans, spy_rtn)
spy_inv = np.reciprocal(spy_rtn_mul)
spy_inv_mul_appl = np.matmul(spy_rtn_trans, appl_rtn)
betaEst = np.multiply(spy_inv, spy_inv_mul_appl)
betaEst
...
def beta_sensitivity(x, y):
    ...
    for i in range(len(x)):
        ...
        dropped_x = np.delete(x, i).reshape(-1,1)
...
beta_sen = beta_sensitivity(spy_rtn, appl_rtn)[:5]
beta_sen